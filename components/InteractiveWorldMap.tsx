...
export function InteractiveWorldMap({ travels }: InteractiveWorldMapProps) {
  ...
  useEffect(() => {
    ...
    scroller.scrollLeft = (scroller.scrollWidth - scroller.clientWidth) * 0.48;
  }, []);
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <div
          className="overflow-x-auto overscroll-x-contain md:overflow-visible"
          ref={mapScrollRef}
        >
          <div className="relative aspect-[1672/941] w-[720px] max-w-none md:min-h-[320px] md:w-full">
            <img
              alt="极简世界地图"
      ...
      <p className="text-center text-xs leading-6 text-[#948879]">
        手机端可以左右滑动查看完整地图；点击地图上的点会跳到对应记录。
      </p>
    </div>
  );
}
